...
plot.load <- function(Variab = Data$Wavenumber[Sample],Load = PCA,PCs = c("PC1","PC2","PC3")){
  
  Variance <- Load$sdev^2/sum(Load$sdev^2) # Berechnung der erklärten varianz pro PC
  
if(Type[Stats] == "Spectra_d"){    # if um zu überprüfen ob Ableitung vorliegt
    Variab = Data$Wavenumber_d[Sample_d] # Abgeleitete Spektren brauchen anderen Wellenzahlen und Bereiche
  }  
...
if(Spektren == "Spectra_d"){    # if um zu überprüfen ob Ableitung vorliegt
  Wellenzahl = Liste$Wavenumber_d # Abgeleitete Spektren brauchen anderen Wellenzahlen und Bereiche
  Bereich = Sample_d
}
